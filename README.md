...
The nginx reverse proxy, `docker-compose.prod.yml`, and deploy scripts moved
out of this folder into `../infra/` (they front both `backend/` and
`frontend/`, so they don't belong to either individually). See
`../infra/README.md` for the full production deploy flow.

This folder still owns `backend/.env`, which `infra/docker-compose.prod.yml`
reads as its single source of truth for secrets and compose `${VAR}`
substitutions - see `example.env` for the full variable list.
```